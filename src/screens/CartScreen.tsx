...
export const CartScreen: React.FC<CartScreenProps> = (props) => {
  const {
    ...
    user, deliveryMethod, paymentMethod, address, needsChange, changeAmount,
    isLocating, isAddressEditing, showAddressError, saveAddressAsDefault,
    isGoogleLoaded,
    onSetDeliveryMethod, onSetPaymentMethod, onSetAddress, onSetNeedsChange,
    onSetChangeAmount, onSetAddressEditing, onSetShowAddressError, onSetSaveAddressAsDefault,
  ...
  if (cart.length === 0) return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="min-h-screen pb-32 bg-white">
      ...
      <div className="flex flex-col items-center justify-center py-20 text-center space-y-4 m-4 bg-white rounded-2xl border border-primary/5 shadow-xl">
        ...
        <p className="font-body text-xs text-on-surface-variant opacity-50 px-10">Que tal adoçar o seu dia com nossas delícias?</p>
        ...
        {!user && (
          <div className="bg-white rounded-2xl p-4 shadow-xl shadow-black/5 border border-primary/5 space-y-3">
            <div className="flex justify-between items-center pb-1">
              <span className="text-[10px] font-black text-primary tracking-widest">Cliente</span>
              <button onClick={() => onNavigate('register')} className="text-primary text-[10px] font-bold">Iniciar sessão</button>
            </div>
            <div className="space-y-2">
              <input type="text" value={guestName} onChange={e => setGuestName(e.target.value)} placeholder="Nome *"
                className="w-full bg-transparent rounded-xl px-4 py-1.5 font-body outline-none border border-primary/10 text-xs" />
              <input type="tel" value={guestPhone} onChange={e => setGuestPhone(e.target.value)} placeholder="Telefone *"
                className="w-full bg-transparent rounded-xl px-4 py-1.5 font-body outline-none border border-primary/10 text-xs" />
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-2xl shadow-xl shadow-black/5 border border-primary/5 p-4 space-y-5">
          ...
          <div className="flex gap-2">
            {[{ id: 'delivery', label: 'Entrega', Icon: Truck }, { id: 'pickup', label: 'Recolha', Icon: Package }].map(({ id, label, Icon }) => (
              <button key={id} onClick={() => onSetDeliveryMethod(id as any)}
          ...
          <AnimatePresence mode="wait">
            {deliveryMethod === 'delivery' && (
              <motion.div key="delivery" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="space-y-4 overflow-hidden">
                <div className="bg-primary/5 p-4 rounded-xl border border-primary/10">
                  ...
                  <p className="text-[11px] text-black font-bold leading-relaxed">Prazo mínimo de 48 horas antes da entrega.</p>
                  <p className="text-[10px] text-black/60 mt-1 italic">Observação: Entregas sob consulta de disponibilidade.</p>
                </div>
                {isAddressEditing ? (
                  <div className="space-y-3">
                    ...
                    <div className={`${showAddressError && !address.address ? 'ring-2 ring-red-500 rounded-xl' : ''}`}>
                      <AddressAutocomplete
                        onSelect={(d) => { onSetAddress({ ...address, ...d }); onSetShowAddressError(false); }}
                        initialValue={address.address} isLoaded={isGoogleLoaded}
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      ...
                      <input type="text" value={address.postal_code} onChange={e => {
                        ...
                        onSetAddress({ ...address, postal_code: v, lat: null, lng: null });
                    ...
                    {user && address.address && (
                      <label className="flex items-start gap-3 rounded-xl border border-primary/10 px-3 py-3">
                        <input type="checkbox" checked={saveAddressAsDefault} onChange={e => onSetSaveAddressAsDefault(e.target.checked)} className="mt-0.5 h-4 w-4 accent-[--color-primary]" />
                        <span className="text-[10px] font-medium text-black/70">Salvar como endereço padrão</span>
                      </label>
                    )}
                    {address.address && (
            ...
            {deliveryMethod === 'pickup' && (
              <motion.div key="pickup" initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                <a 
                  href="https://www.google.com/maps/search/?api=1&query=Rua+Miguel+Ângelo+21+1A+Casal+do+Marco+2840-136"
                  target="_blank"
                ...
                >
                  ...
                  <div className="pl-6">
                    <p className="text-[11px] font-bold text-black">Rua Miguel Ângelo, 21, 1 A</p>
        ...
        <div className="bg-white rounded-2xl shadow-xl shadow-black/5 border border-primary/5 p-4 space-y-4">
          ...
          <div className="flex flex-col gap-2">
            {PAYMENT_METHODS.map((m) => (
              <button key={m.id} onClick={() => onSetPaymentMethod(m.id as any)}
                className={`p-3 rounded-xl border transition-all flex items-center justify-between gap-3 ${paymentMethod === m.id ? 'bg-primary/5 border-primary shadow-sm' : 'border-primary/10'}`}>
                <div className="flex items-center gap-3">
                  <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${paymentMethod === m.id ? 'border-primary' : 'border-primary/20'}`}>
                  ...
                  <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                    <div className="relative">
                      ...
                      <input type="text" inputMode="decimal" value={changeAmount} onChange={e => onSetChangeAmount(e.target.value)} placeholder="0,00"
                        className="w-full bg-white border border-primary/10 rounded-xl pl-8 pr-4 py-3 text-sm font-black text-primary outline-none" />
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
        ...
        <section className="bg-white border border-primary/5 p-6 rounded-2xl shadow-xl space-y-5">
          ...
          <div className="space-y-3">
            <div className="flex justify-between text-[10px]">
              <span className="font-bold text-black/60">Artigos ({cart.reduce((s, i) => s + i.quantity, 0)})</span>
